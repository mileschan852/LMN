...
export interface SupabaseClient {
  ...
  fetchNearby: (lat: number, lng: number, radiusDegrees?: number) => Promise<DbUser[]>
...
export function createSupabaseClient(tableName: string): SupabaseClient {
  return {
    ...
    fetchNearby: (lat, lng, radiusDegrees = 0.1) => fetchNearby(tableName, lat, lng, radiusDegrees),
